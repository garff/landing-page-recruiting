...
export default function BlogPage() {
  ...
  const getTranslatedPost = (post: BlogPost) => {
    return {
      ...
      excerpt: language === 'da' && post.excerptDa ? post.excerptDa : post.excerpt,
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* Hero Section */}
      <section className="bg-primary/5 py-16 md:py-24" aria-labelledby="blog-heading">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto">
            <h1 id="blog-heading" className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6">
              {t.blog.title}
            </h1>
      ...
      <section className="py-8 border-b" aria-labelledby="category-heading">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 id="category-heading" className="sr-only">Filter by category</h2>
      ...
      {/* Featured Post */}
      {filteredPosts.find(post => post.featured) && selectedCategory === t.blog.categories.all && (
        <section className="py-12 bg-background" aria-labelledby="featured-heading">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 id="featured-heading" className="text-2xl font-bold mb-8">{t.blog.featuredArticle}</h2>
            {filteredPosts
                  ...
                  <div className="space-y-4">
                    ...
                    <Link href={`/blog/${post.id}`}>
                      <Button className="mt-2">
                        {t.blog.readArticle} <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" />
                      </Button>
                    </Link>
                  </div>
                </div>
      ...
      {/* Blog Posts Grid */}
      <section className="py-12 bg-background flex-grow" aria-labelledby="articles-heading">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 id="articles-heading" className="text-2xl font-bold mb-8">
            {selectedCategory !== t.blog.categories.all ? `${selectedCategory} ${t.blog.latestArticles}` : t.blog.latestArticles}
          </h2>
          
          {filteredPosts.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-lg text-muted-foreground">{t.blog.posts.noArticlesFound}</p>
              <Button 
                variant="outline" 
                className="mt-4"
                onClick={() => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredPosts
                .filter(post => !post.featured || selectedCategory !== t.blog.categories.all)
                .map(post => (
